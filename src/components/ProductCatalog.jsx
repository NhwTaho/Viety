  ...
  return (
    <section id="productsSection" className="py-14 bg-white scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="px-3.5 py-2 text-xs font-semibold bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-600 text-slate-700 cursor-pointer"
            >
        ...
        {/* CATEGORY SUBTABS */}
        <div className="flex items-center gap-2 overflow-x-auto pb-4 mb-8 no-scrollbar scroll-smooth flex-nowrap">
          {CATEGORIES.map((cat) => (
            <button
              key={cat.id}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map((product) => (
              <div
                ...
                className="bg-white rounded-2xl border border-slate-200/90 overflow-hidden shadow-xs hover:shadow-xl hover:border-slate-300 transition-all duration-300 flex flex-col group hover:-translate-y-1 cursor-pointer"
              >
                {/* IMAGE CONTAINER */}
                <div className="relative aspect-4/3 bg-slate-50 overflow-hidden">
                  <span className="absolute top-3 left-3 z-10 px-2.5 py-1 text-[11px] font-extrabold bg-white/90 backdrop-blur-md text-slate-800 rounded-full border border-slate-200/80 shadow-xs">
                    {product.badge}
                  </span>
                  ...
                  <button
                    ...
                    className="absolute top-3 right-3 z-10 p-2 bg-white/90 backdrop-blur-md hover:bg-white text-slate-700 hover:text-blue-600 rounded-full shadow-md transition opacity-80 group-hover:opacity-100 cursor-pointer"
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        ...
                        className="p-2 bg-red-600 hover:bg-red-700 text-white rounded-xl shadow-md shadow-red-600/20 transition active:scale-95 flex items-center gap-1 text-xs font-semibold cursor-pointer"
                        title="Thêm vào giỏ"
                      >
